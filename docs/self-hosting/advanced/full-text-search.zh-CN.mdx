...
命令会输出 run ID，以及类似 `<namespace>-messages-v1-r<run-id>` 的索引名。若任务中断，使用相同命令并增加 `--run-id=<run-id>` 继续。持续同步会同时写入现有 generation 和重建 generation；不要暂停应用写入或同步 worker。回填显示 `ready_for_incremental_sync` 且 `messages` Outbox 空闲后，按精确物理索引名切换：
...
alias 和逻辑 schema 版本保持不变。观察期内保留旧 generation，出现问题时可按其精确名称切回。确认无需回滚后，再依次执行 `--retire --entity=messages --yes` 和 `--purge --entity=messages --yes`。
