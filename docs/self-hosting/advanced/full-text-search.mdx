...
generations; do not pause application writes or the sync worker. After the backfill reports
...
observation window so the alias can be promoted back to its exact name. Run
`--retire --entity=messages --yes` and later `--purge --entity=messages --yes` only after rollback is
no longer needed.
